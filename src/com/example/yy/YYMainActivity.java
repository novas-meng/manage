...
import android.widget.AdapterView.OnItemSelectedListener;
   public class YYMainActivity extends Activity {
   ...
   private static final String[] costs={"物业费","车位费","电梯费","供暖费","电费","水费","燃气费","宽带通讯费"};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listview=(ListView)findViewById(R.id.yy_listview);
        ...
        if(bundle!=null)
        {
        	   if(bundle.size()==2)
               {
        		...
        		//System.out.println("yytype="+bundle.getString("type"));
    ...
    //    type=1;
        ...
        adapter=new yyInformationAdapter(list,YYMainActivity.this,new SpinnerSelectedListener());
        listview.setAdapter(adapter);       
        //setLayout(type);  
        isConnected=NetStateCheck.isNetConnected(YYMainActivity.this);
       // if(isConnected)
  	...
  	Handler ButlerYYHandler=new Handler()
  	{
  		...
  		public void handleMessage(Message msg) {
  			...
  			if(msg.what==0)
  			{
  				type=1;
  				setLayout(type);
  				Toast.makeText(YYMainActivity.this,"获取用户列表失败，请检查一下网络连接状态",Toast.LENGTH_SHORT).show();
  			}
  			else
  			{
  				...
  				else
  				{
  					try {
  	  	    			...
  	  	    			{		
  	  	    			  ...
  	  	    			  System.out.println("username="+username+"content="+content+"money="+money+"_id="+id);
    ...
    Handler messagehandler=new Handler()
    {
		...
		public void handleMessage(Message msg) {
			...
			{
            	Toast.makeText(YYMainActivity.this,"预约消息发送失败",Toast.LENGTH_SHORT).show();
			}
			else
			{
			//	YYDataStore.saveYYMessage(0,sendText);
            	Toast.makeText(YYMainActivity.this,"预约消息发送成功",Toast.LENGTH_SHORT).show();
			}
		}    	
    };
    ...
    public boolean check()
    {
    	...
    	if(timeEdit.getText().toString().equals("")||monthEdit.getText().toString().equals("")||dayEdit.getText().toString().equals(""))
    	{
    		System.out.println("信息添加不完整");
    		Toast.makeText(YYMainActivity.this,"您的日期还没有添加完整",Toast.LENGTH_SHORT).show();
    		return false;
    	}
    	int month=0;
    	int day=0;
        ...
        {
        	Toast.makeText(YYMainActivity.this,"输入的日期格式不对哦",Toast.LENGTH_SHORT).show();
        }
    	if(month>12||month<1||day<1||day>31||time<1||time>24)
    	{
    		Toast.makeText(YYMainActivity.this,"您的日期添加的格式错误",Toast.LENGTH_SHORT).show();
    	}
    	return true;
    }
      ...
      Handler delInformationHandler=new Handler()
     {
		...
		public void handleMessage(Message msg) {
			...
			if(msg.what==1)
			{
				...
				Toast.makeText(YYMainActivity.this.getApplicationContext(),"删除预约消息成功",Toast.LENGTH_SHORT).show();
			}
			else
			{
				System.out.println("删除预约消息失败");
				Toast.makeText(YYMainActivity.this,"删除预约消息失败，请检查一下网络设置",Toast.LENGTH_SHORT).show();
			}
		} 
     };
    //删除信息的runnable
    ...
    public void delInformation(View v)
    {
    	...
    	{
    		System.out.println("删除的内容为"+v.getContentDescription().toString());
