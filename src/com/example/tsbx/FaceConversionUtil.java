...
//		String zhengze = "\\[[^\\]]+\\]";
	 ...
	 * @return
	 */
	public SpannableString addFace(Context context, String imgPath,
		...
		int imgId=context.getResources().getIdentifier(imgPath,"drawable", context.getPackageName());
		spannableString="{"+imgPath+":"+spannableString+"}";
...
//				imgId);
	...
	private void ParseData(List<String> data, Context context) {
		...
		try {
			...
			//System.out.println("entryname="+context.getResources().getResourceEntryName(resID));
				//System.out.println("name="+context.getResources().getResourceEntryName(resID));
				if (resID != 0) {
					emojEentry = new ChatEmoji();
					emojEentry.setId(resID);
					emojEentry.setCharacter(text[1]);
					emojEentry.setFaceName(fileName);
					emojEentry.setEntryName(entryName);
					emojis.add(emojEentry);
				}
			}
			int pageCount = (int) Math.ceil(emojis.size() /14 + 0.1);
